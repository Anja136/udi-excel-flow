...
const ConfigStep: React.FC<ConfigStepProps> = ({
  ...
  const templates = [{
    id: "template1",
    name: "UDI-DI Template"
  }, {
    id: "template2",
    name: "Basic Device Information"
  }, {
    id: "template3",
    name: "Full Device Details"
  }, {
    id: "template4",
    name: "Package Labeling Template"
  }];
  const dataSources = [{
    id: "source1",
    name: "Enterprise GUDID"
  }, {
    id: "source2",
    name: "Master Device Registry"
  }, {
    id: "source3",
    name: "Clinical Systems"
  }, {
    id: "source4",
    name: "Legacy Product Database"
  }];
  ...
  useEffect(() => {
    if (savedProcesses.length > 0) {
      const matchingProcess = savedProcesses.find(process => process.authority === selectedConfig.authority && process.template === selectedConfig.template && process.dataSource === selectedConfig.dataSource);
      if (matchingProcess) {
  ...
  return <div className="max-w-3xl mx-auto">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select a saved template or create new one's throug own download parameters:</CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="bg-muted/40 p-4 rounded-lg border border-muted">
            ...
            <Select value={selectedProcess} onValueChange={handleProcessSelect}>
              ...
              <SelectContent>
                {savedProcesses.length > 0 ? savedProcesses.map(process => <SelectItem key={process.id} value={process.id}>
                      {process.name}
                    </SelectItem>) : <SelectItem value="no-processes" disabled>
                    No saved processes
                  </SelectItem>}
                {selectedProcess && <SelectItem value="configure">
          ...
          {!showDetailConfig && <div className="flex justify-center mt-4">
              <Button variant="outline" onClick={() => setShowDetailConfig(true)} className="flex items-center gap-2">
          ...
          {showDetailConfig && <div className="space-y-6 mt-6">
              <div className="space-y-2">
                <Label htmlFor="authority">Regulatory Authority</Label>
                <Select value={selectedConfig.authority} onValueChange={value => handleChange('authority', value)}>
                  <SelectTrigger id="authority">
                    <SelectValue placeholder="Select Authority" />
                  </SelectTrigger>
                  <SelectContent>
                    {authorities.map(authority => <SelectItem key={authority.id} value={authority.id}>
                        {authority.name}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-2">
                ...
                <Select value={selectedConfig.template} onValueChange={value => handleChange('template', value)}>
              ...
              <div className="space-y-2">
                ...
                <Select value={selectedConfig.dataSource} onValueChange={value => handleChange('dataSource', value)}>
              ...
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                ...
                <DialogContent>
                  ...
                  <div className="py-4">
                    ...
                    <Input id="processName" value={newProcessName} onChange={e => setNewProcessName(e.target.value)} placeholder="Enter process name" />
                  </div>
                  <DialogFooter>
